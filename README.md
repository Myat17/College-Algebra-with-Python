# College-Algebra-with-Python
This repository contains my notes, exercises, and projects completed while studying the **College Algebra with Python** course from FreeCodeCamp.

## Course Information

- Platform: FreeCodeCamp
- Course: College Algebra with Python
- Language: Python

## Learning Objectives

- Learn Python fundamentals through algebra
- Solve algebraic equations using Python
- Understand functions and graphs
- Practice mathematical problem solving
- Improve Python programming skills

## Repository Structure

```
lesson_01/ Proportions, ratios and conversions
lesson_02/ Basic Algebra, solving one variable equations
lesson_03/ Fractions, percents and decimals
lesson_04/ Functions and using two variables (x,y)
lesson_05/ Slope and intercept on a graph
lesson_06/ 
```

## Progress

- [x] Lesson 1
- [x] Lesson 2
- [x] Lesson 3 
- [x] Lesson 4
- [ ] Lesson 5

## Tools

- Python 3.12
- Visual Studio Code
- Git
- GitHub

## Reference

freecodecamp.org YouTube Channel
